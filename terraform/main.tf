...
data "aws_acm_certificate" "existing_cert" {
  ...
  domain   = "*.tomast.it" # Replace with your domain
  statuses = ["ISSUED"]
}

locals {
  aliases = var.use_custom_domain && var.root_domain != "" ? [
    "digitaltwin.${var.root_domain}"
  ] : []
...
resource "aws_lambda_function" "api" {
  ...
  environment {
    variables = {
      CORS_ORIGINS     = var.use_custom_domain ? "https://digitaltwin.${var.root_domain}" : "https://${aws_cloudfront_distribution.main.domain_name}"
...
resource "aws_cloudfront_distribution" "main" {
  ...
  viewer_certificate {
    acm_certificate_arn            = var.use_custom_domain ? data.aws_acm_certificate.existing_cert.arn : null
